...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const popupQR = document.querySelector("#popupQR");
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "Escape" && popupQR && popupQR.style.display === "flex") {
      hideDialogDonate();
    }
  });
});
...
function initializeScrollAnimations() {
  ...
  document
    .querySelectorAll(".slide-in, .story-item, .info-card, .countdown-item")
    .forEach((el) => {
      observer.observe(el);
    });
}
...
function submitRSVP(formData) {
  ...
  const scriptURL = "https://script.google.com/macros/s/AKfycbwXYWbdtrouSlPRdEBdppSPWP3kYxZUUTVwYN-ZzlIU8L3cLOv-T6hFYqpW9e_IRQG3/exec";
...
function initializeAdvancedAnimations() {
  ...
  document
    .querySelectorAll(
      ".countdown-item, .photo-item, .story-content, .info-card"
    )
    .forEach((el) => {
...
function showDialogDonate() {
  ...
  if (popupQR) {
    ...
    const modalContent = popupQR.querySelector(".qr-modal-content");
    const closeBtn = popupQR.querySelector(".qr-close");
    const qrCards = popupQR.querySelectorAll(".qr-card");
    const qrCodes = popupQR.querySelectorAll(".qr-code");

    // Modal content animation
    modalContent.className =
      "qr-modal-content animate__animated animate__zoomIn";

    // Close button animation
    setTimeout(() => {
      closeBtn.className = "qr-close animate__animated animate__rotateIn";
    }, 300);

    // QR cards animation
    qrCards.forEach((card, index) => {
      setTimeout(() => {
        card.className = "qr-card animate__animated animate__slideInUp";
      }, 500 + index * 200);
    });
    ...
    qrCodes.forEach((code, index) => {
      setTimeout(() => {
        code.className = "qr-code animate__animated animate__bounceIn";
      }, 900 + index * 200);
    });
  }
}
...
function initializeCustomCursor() {
  ...
  const clickableElements = document.querySelectorAll(
    "a, button, .photo-item, .heart-button, .music-player, .countdown-item, .story-content, .info-card"
  );
